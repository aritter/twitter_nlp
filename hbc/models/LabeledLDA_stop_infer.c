...
code can be built using the following command:
  ...
  gcc -O3 -lm labels.c stats.c samplib.c LabeledLDA_stop_infer.c -o LabeledLDA_stop_infer.out
...
void resample_post_theta(int D, int K, int** M, int* N, double** post_theta, double** post_theta_train, int *entity_map, int*** z) {
  int d_9;
  double* tmpSP7;
  int n_3;
  int m_75;
  ...
  for (d_9=1; d_9<=D; d_9++) {
    ...
    if((entity_map == NULL) || (entity_map[d_9-1] == -1)) {
...
void resample_s(int D, int** M, int* N, double alpha, double eta, double psi, double phi, double** post_beta, double** post_theta, double** post_theta_train, int* entity_map, double *post_gamma, double* post_pi, int*** w, int*** z, int*** s, int** labels, int K, int V) {
  int d_10;
  int n_16;
  int m_22;
  double* post_s;
    ...
    for (n_16=1; n_16<=N[d_10-1]; n_16++) {
      for (m_22=1; m_22<=M[d_10-1][n_16-1]; m_22++) {
        ...
        post_s[0] = (ldf_Mult_smooth(0, eta, w[d_10-1][n_16-1][m_22-1], post_beta[z[d_10-1][n_16-1][m_22-1]-1], 1, V)) + 
          //(ldf_Mult_smooth(0, alpha, z[d_10-1][n_16-1][m_22-1], theta_counts, 1, K)) +	//Oops!
...
void resample_z(int D, int** M, int* N, double alpha, double eta, double psi, double** post_beta, double** post_theta, double** post_theta_train, int* entity_map, int*** w, int*** z, int*** s, int** labels, int K, int V) {
  int d_10;
  int n_16;
  int m_22;
  double* tmp_post_z_3;
  ...
  for (d_10=1; d_10<=D; d_10++) {
    ...
    for (n_16=1; n_16<=N[d_10-1]; n_16++) {
      for (m_22=1; m_22<=M[d_10-1][n_16-1]; m_22++) {
        ...
        theta_counts[z[d_10-1][n_16-1][m_22-1]-1] -= 1.0;
	assert(theta_counts[z[d_10-1][n_16-1][m_22-1]-1] >= 0);

        /* Sample */
        z[d_10-1][n_16-1][m_22-1] = sample_Mult_labeled_stop(w[d_10-1][n_16-1][m_22-1], s[d_10-1][n_16-1][m_22-1], post_beta, theta_counts, V, K, alpha, eta, labels[d_10-1], labelIdx, post_z);
...
void initialize_post_theta(double** post_theta, double** post_theta_train, int *entity_map, int D, int K, int** M, int* N, int*** z) {
  int dvv_loop_var_1;
  int dvv_loop_var_2;
  for (dvv_loop_var_1=1; dvv_loop_var_1<=D; dvv_loop_var_1++) {
  ...
  resample_post_theta(D, K, M, N, post_theta, post_theta_train, entity_map, z);
}
...
void zero_counts_stop(double *post_gamma, double* post_pi, int V)
{
  int k;
  int v;
...
void increment_counts_stop(double *post_gamma, double *post_pi, int D, int *N, int **M, int V, int ***w, int ***s)
{
  int d;
  int m;
  int n;
...
int main(int ARGC, char *ARGV[]) {
  ...
  if(ARGC != 9) {
    fprintf(stderr, "usage %s <train_w> <train_z> <train_s> <w> <labels> <entityMap> <nIter> <dumpInterval>\n", ARGV[0]);
    exit(1);
  }
  ...
  s_train = load_discrete3(ARGV[3], &D_train, &N_train, &M_train, &tmp);

  /* variables defined with --loadD */
  w = load_discrete3(ARGV[4], &D, &N, &M, &V_tmp);
  labels = load_labels(ARGV[5], &D, &K);
  entity_map = load_entity_map(ARGV[6], D);
  nIter = atoi(ARGV[7]);
  dumpInterval = atoi(ARGV[8]);

  /* Use maximum vocab size between train and test sets */
  V = V > V_tmp ? V : V_tmp;
  ...
  initialize_post_theta(post_theta_train, NULL, NULL, D_train, K, M_train, N_train, z_train);
  initialize_post_theta(post_theta, post_theta_train, entity_map, D, K, M, N, z);
  ...
  zero_counts_stop(post_gamma, post_pi, V);
  increment_counts_stop(post_gamma, post_pi, D_train, N_train, M_train, V, w_train, s_train);
  increment_counts_stop(post_gamma, post_pi, D, N, M, V, w, s);
  ...
  ******************************************************/
  ...
  for (iter=1; iter<=nIter; iter++) {
    ...
    fflush(stderr);
    resample_z(D, M, N, alpha, eta, psi, post_beta, post_theta, post_theta_train, entity_map, w, z, s, labels, K, V);
    resample_s(D, M, N, alpha, eta, psi, phi, post_beta, post_theta, post_theta_train, entity_map, post_gamma, post_pi, w, z, s, labels, K, V);
